...
export async function action({ request, context, params }: ActionFunctionArgs) {
  ...
  if (!draft) {
    return Response.json({ error: "Choose a supported rights basis.", intent }, { status: 400 });
  }
  if (intent === "save-draft") {
    const result = await service.saveDraft(tokenHash, draft, abuseMeta);
    if (!result.ok) {
      return Response.json(
        { error: result.error.message, intent },
        { status: submissionHttpStatus(result.error.code) },
      );
    }
...
export default function SubmissionRoute() {
  ...
  const actionData = useActionData<{ error?: string; intent?: string }>();
  ...
  const showDetailsEditor = editingDetails || actionData?.intent === "save-draft";
          ...
          {showDetailsEditor ? (
            ...
            <div className="submission-saved-summary">
              <div>
                ...
                <p>Details saved. Continue with the private listening copy below.</p>
              </div>
              {detailsEditable ? (
