...
Configure `ACCESS_TEAM_DOMAIN`, `ACCESS_AUD`, `CURATOR_EMAILS`, `DATABASE_URL`, `MEDIA_DELIVERY_SIGNING_SECRET`, and the `MEDIA_BUCKET` R2 binding externally in the Cloudflare Worker dashboard. The repository contains the non-secret Cloudflare account ID and R2 bucket names in `wrangler.jsonc`, but never credential values. Actor subject and email are retained for audits. Run `npm run typegen` after every Wrangler configuration change.
...
_Note: Cloudflare Hotlink Protection is for common image formats such as GIF, ICO, JPG, JPEG, and PNG. It does not protect public MP3/MP4 URLs. Before uploading real audio or video, use private storage plus signed URLs, Workers, Stream, or custom WAF logic for media delivery._
...
_Note: Cloudflare WAF rules do not add response headers. Configure response header modifications with Transform Rules or set headers at the origin, then verify them in browser dev tools._
