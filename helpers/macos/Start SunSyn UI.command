...
if command -v lsof >/dev/null 2>&1 && lsof -nP -iTCP:5173 -sTCP:LISTEN >/dev/null 2>&1; then
  die "Port 5173 is already in use. Quit the process using it and launch the UI again."
fi

print -r -- "Launching the fixture-backed SunSyn UI at http://127.0.0.1:5173"
print -r -- "This uses test mode so the repository's fixture-backed runtime stays isolated from online data."

int_received=0
trap 'int_received=1' INT
set +e
npm run dev -- --mode test --host 127.0.0.1 --port 5173 --strictPort --open
exit_status=$?
set -e
trap - INT

if (( int_received )); then
  print -r -- "Development server stopped."
  pause_for_readability
  exit 0
fi

if (( exit_status != 0 )); then
  die "Development server stopped with exit code $exit_status."
fi
